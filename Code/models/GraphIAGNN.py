...
class IAGNNBB(torch.nn.Module):
    def __init__(self, args):
        super(IAGNNBB, self).__init__()
        ...
        self.model_c = EModel_attention(args)
        self.model_b = EModel_attention(args)
    ...
    def forward(self, data):
#        x, edge_index, batch = data.x, data.edge_index, data.batch
        z_c = self.model_c(data)
        z_b = self.model_b(data)
